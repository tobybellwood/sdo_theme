...
function sdo_theme_preprocess_field_field_decision_legal_name(&$variables) {
  ...
  // Make sure we have access to the field for the provider id.
  if (empty($node->field_provider[LANGUAGE_NONE][0]['entity'])) {
    return;
  }

  $provider = $node->field_provider[LANGUAGE_NONE][0]['entity'];
  $link = l($name, "http://training.gov.au/Organisation/Details/{$provider->field_rto_id[LANGUAGE_NONE][0]['value']}");
...
function sdo_theme_preprocess_block(&$variables) {
  ...
  if ($elements['#block']->region == 'sidebar' && $elements['#block']->module == 'menu') {
    ...
    $elements['#children'] = $variables['content'] = theme('container', array(
      'element' => array(
        '#children' => $elements['#children'],
        '#attributes' => array(
          'class' => 'menu-block-wrapper menu-block-govcms_menu_block-sidebar',
        ),
      )
    ));
  }
...
function sdo_theme_preprocess_views_view_table(&$variables) {
  ...
  $variables['attributes_array']['data-tablesaw-sortable'] = '';
  $variables['attributes_array']['data-tablesaw-mode-switch'] = '';

  foreach ($variables['header'] as $header => $label) {
    $idx++;

    if (in_array($header, $persistent_headers)) {
      $variables['header_attributes'][$header] = [
    ...
    $variables['header_attributes'][$header] = [
      'data-tablesaw-priority' => ($idx > 5) ? '6' : '1',
    ];
  }

  // Add the tablesaw library to help display tables better.
  // @see https://github.com/filamentgroup/tablesaw
  drupal_add_js("$path/js/lib/tablesaw.js");
  drupal_add_js("$path/js/lib/tablesaw.init.js");
  drupal_add_css("$path/js/lib/tablesaw.css");
}
